...
failed_stocks = [] # <-- NEW: Create a list to remember which ones fail
...
for ticker in ticker_symbols:
    try:
        ...
        if len(stock_prices) < 200:
            failed_stocks.append(ticker) # Track it if it's too new/doesn't have enough data
            continue 
        ...
        failed_stocks.append(ticker) # Track the exact symbol if Yahoo throws an error

df = pd.DataFrame(portfolio_data)
print(f"✅ Successfully processed {len(df)} live stocks!")

# --- NEW: PRINT THE SUMMARY OF MISSED STOCKS ---
if failed_stocks:
    print(f"\n⚠️ NOTE: The following {len(failed_stocks)} stocks were skipped (delisted, symbol changed, or missing data):")
